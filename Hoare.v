...
Example assn_example_three : {{assn_sub X (ANum 3) (fun st => 0 <= st X /\ st X <= 5)}}
...
(* Doesn't work for a based around X, such as a = X + 1 *)
(** [] *)
...
Proof.
  ...
  unfold hoare_triple. intros. destruct H0. subst. inversion H. subst. 
  assert (st = update (update st X (aeval st a)) X (st X)) as Eq.
  apply functional_extensionality. intros. rewrite update_shadow.
  rewrite update_same; reflexivity. rewrite <- Eq. split. apply H0.
  reflexivity.
Qed.
...
Example hoare_asgn_examples_2_1 : {{fun st => st X + 1 <= 5}}
  (X ::= APlus (AId X) (ANum 1)) {{fun st => st X <= 5}}.
Proof.
  eapply hoare_consequence_pre. apply hoare_asgn. intros st H. 
  unfold assn_sub. rewrite update_eq. simpl. apply H.
Qed.

Example hoare_asgn_examples_2_2 : {{fun _ => 0 <= 3 /\ 3 <= 5}}
  (X ::= ANum 3) {{fun st => 0 <= st X /\ st X <= 5}}.
Proof.
  eapply hoare_consequence_pre. apply hoare_asgn. intros st H. 
  unfold assn_sub. rewrite update_eq. simpl. apply H.
...
Proof.
  ...
  apply hoare_asgn. split. reflexivity. reflexivity.
Qed.
...
(* If a is given X, the expression's result will change. *)
(** [] *)
...
Proof.
  ...
  destruct H. apply ble_nat_true in H0. destruct H0; admit. admit.
Qed.
...
Proof.
  intros Q X st st' a b. inversion a. apply b.
Qed.
